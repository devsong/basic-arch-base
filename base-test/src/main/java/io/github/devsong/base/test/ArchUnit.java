...
@AnalyzeClasses(
        packages = GlobalConstant.SYSTEM_PACKAGE_PREFIX,
        importOptions = {ImportOption.OnlyIncludeTests.class})
public class ArchUnit extends BaseTest {

    @ArchTest
    protected final ArchRule test_class_should_be_package_private =
            classes().that().haveSimpleNameEndingWith("Test").should().bePackagePrivate();

    @ArchTest
    protected final ArchRule test_method_should_be_package_private = methods()
            .that()
